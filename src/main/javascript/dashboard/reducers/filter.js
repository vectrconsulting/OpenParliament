...
export const filterData = (data, columnFilters, dateFilters, search_string) => {
    const filter_func = (row) => {
        if (row.date < dateFilters.start || dateFilters.end < row.date)
            return false;
        for (const filter of columnFilters) {
            if (!filter.values.includes(row[filter.columnName])) return false;
        }
        if (search_string !== undefined && search_string !== "")
            return row.department.includes(search_string) || row.author.includes(search_string) || row.question.includes(search_string) || row.answer.includes(search_string) ||row.title.includes(search_string);
        return true;
    };
