...
export class QuestionTable extends Component {
    ...
    body() {
        const rows = this.props.questions
            ...
            .map((row, index) => (
                <tr key={`opendata-table-row-${index}`} onClick={() => this.setState({ show_modal: true, data_for_modal: row })}>
                    {
    ...
    modal() {
        if (this.state.show_modal)
            return (
                <Modal visible={true} onClickBackdrop={() => this.setState({ show_modal: false, data_for_modal: undefined })}>
                    <div className="modal-header">
                        <h5 className="modal-title">
                            {this.state.data_for_modal.title}
                        </h5>
                        <button type="button" className="close" onClick={() => this.setState({ show_modal: false, data_for_modal: undefined })}>
                    ...
                    <div className="modal-footer">
                        <button type="button" className="btn btn-secondary" onClick={() => this.setState({ show_modal: false, data_for_modal: undefined })}>
                            {this.props.close}
                        </button>
                    </div>
                </Modal>
...
export default connect(
    state => ({
        questions: filterData(state.data.questions.items, state.filter.columns, state.filter.dates)
